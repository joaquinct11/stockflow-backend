...
class ReportesServiceImplTest {
    ...
    void setUp() {
        ...
        when(movimientoRepository.sumCantidadByTenantIdAndTipoAndPeriodo(eq(TENANT), eq("SALIDA"),  any(), any(), any())).thenReturn(0L);
        when(movimientoRepository.findTopMovimientosProductos(eq(TENANT), any(), any(), any())).thenReturn(Collections.emptyList());

        // recepciones sin datos (ahora con sucursalId)
        when(recepcionRepository.countConfirmadasByTenantIdAndPeriodo(eq(TENANT), any(), any(), any())).thenReturn(0L);
        when(recepcionRepository.sumUnidadesRecibidasByTenantIdAndPeriodo(eq(TENANT), any(), any(), any())).thenReturn(0L);
        when(recepcionRepository.sumMontoComprasByTenantIdAndPeriodo(eq(TENANT), any(), any(), any())).thenReturn(null);
    }

    @Test
    void obtenerResumen_sinDatos_devuelveEstructuraCompleta() {
        when(ventaRepository.countByTenantIdAndPeriodo(eq(TENANT), any(), any(), any())).thenReturn(0L);
    ...
    void obtenerResumen_conInventario_devuelveProductosBajoStock() {
        ...
        Producto prod = Producto.builder()
                .id(1L).nombre("Producto A").stockActual(2).stockMinimo(10).tenantId(TENANT).build();
    ...
    void obtenerResumen_conVentas_devuelveSeccionVentas() {
        when(ventaRepository.countByTenantIdAndPeriodo(eq(TENANT), any(), any(), any())).thenReturn(3L);
        when(ventaRepository.sumTotalByTenantIdAndPeriodo(eq(TENANT), any(), any(), any())).thenReturn(new BigDecimal("300.00"));
        when(ventaRepository.findTopProductosVendidos(eq(TENANT), any(), any(), any())).thenReturn(Collections.emptyList());
        when(ventaRepository.sumCostoVentasByTenantIdAndPeriodo(eq(TENANT), any(), any(), any())).thenReturn(new BigDecimal("150.00"));
    ...
    void tendenciaVentas_sinDatos_devuelveListaVacia() {
        when(ventaRepository.findTendenciaDiaria(eq(TENANT), any(), any(), any()))
                .thenReturn(Collections.emptyList());

        List<VentaTendenciaDTO> resultado = reportesService.tendenciaVentas(TENANT, null, DESDE, HASTA, "DIA");
    ...
    void tendenciaVentas_agrupacionMes_llamaQueryMensual() {
        ...
        when(ventaRepository.findTendenciaMensual(eq(TENANT), any(), any(), any()))
                .thenReturn(List.<Object[]>of(row));

        List<VentaTendenciaDTO> resultado = reportesService.tendenciaVentas(TENANT, null, DESDE, HASTA, "MES");
    ...
    void slowMovers_conProductos_devuelveListaConCostoTotal() {
        Producto p1 = Producto.builder()
                .id(10L).nombre("Producto Lento").stockActual(100)
                .costoUnitario(new BigDecimal("15.50")).tenantId(TENANT).activo(true).build();
        Producto p2 = Producto.builder()
                .id(11L).nombre("Otro Lento").stockActual(50)
                .costoUnitario(new BigDecimal("20.00")).tenantId(TENANT).activo(true).build();
    ...
    void slowMovers_conLimit_respetaLimite() {
        List<Producto> muchos = java.util.stream.IntStream.range(0, 10)
                .mapToObj(i -> Producto.builder()
                        .id((long) i).nombre("P" + i).stockActual(5)
                        .costoUnitario(new BigDecimal("10.00")).tenantId(TENANT).activo(true).build())
    ...
    void ventasPorVendedor_sinDatos_devuelveListaVacia() {
        ...
        List<VentaVendedorDTO> resultado = reportesService.ventasPorVendedor(TENANT, null, DESDE, HASTA, 20);
    ...
    void clasificacionAbc_sinDatos_devuelveListaVacia() {
        when(ventaRepository.findTopProductosVendidosPorIngresos(eq(TENANT), any(), any(), any()))
                .thenReturn(Collections.emptyList());

        List<ProductoAbcDTO> resultado = reportesService.clasificacionAbc(TENANT, null, DESDE, HASTA, 200);
    ...
    void clasificacionAbc_respetaLimit() {
        ...
        List<ProductoAbcDTO> resultado = reportesService.clasificacionAbc(TENANT, null, DESDE, HASTA, 2);
    ...
    void ventasPorMetodoPago_sinDatos_devuelveListaVacia() {
        when(ventaRepository.findVentasPorMetodoPago(eq(TENANT), any(), any(), any()))
                .thenReturn(Collections.emptyList());

        List<VentaMetodoPagoDTO> resultado = reportesService.ventasPorMetodoPago(TENANT, null, DESDE, HASTA);
